...
from src.training.dataset_contract import source_family_id, split_for_family
...
PATTERNS: tuple[tuple[str, re.Pattern[str], float], ...] = (
    (
        "KAR_PAYI_ORANI",
        re.compile(
            r"(?:%\s*\d+(?:[,.]\d+)?\s*k[âa]r\s+payı(?:\s+oranı)?|"
            r"k[âa]r\s+payı(?:\s+oranı)?\s*%\s*\d+(?:[,.]\d+)?)",
            re.I,
        ),
        0.99,
    ),
    ...
    (
        "VADE",
        re.compile(
            r"(?<!\d)\d{1,3}\s*(?:ay|yıl)(?:a|e)?\s*"
            r"(?:kadar|varan|vade(?:li|yle)?)?",
            re.I,
        ),
        0.94,
    ),
    (
        "TAKSIT_SAYISI",
        re.compile(r"(?<!\d)\d{1,3}\s*(?:aya\s+varan\s+)?taksit", re.I),
        0.98,
    ),
    (
        "ALISVERIS_PUANI",
        re.compile(
            rf"(?:{MONEY}\s*(?:değerinde\s+)?(?:Worldpuan|Sağlam\s*Puan|puan)|"
            rf"\d[\d.]*\s*(?:Worldpuan|Sağlam\s*Puan))",
            re.I,
        ),
        0.98,
    ),
    (
        ...
        re.compile(
            ...
            rf"\d{{1,2}}\s+{MONTH}\s+20\d{{2}}(?:\s*(?:[-–]|ile)\s*"
    ...
    (
        "HEDEF_KITLE",
        re.compile(
            r"(?:yeni|mevcut|bireysel|ticari|maaş|emekli|genç|öğrenci)\s+"
            r"müşteri(?:lerimiz|ler|miz|si)?|kart\s+sahipleri",
            re.I,
        ),
        0.93,
    ),
    (
        "TAHSIS_UCRETI",
        re.compile(
            r"(?:tahsis\s+ücreti(?:\s+(?:alınmamaktadır|yoktur|muafiyeti))?)",
            re.I,
        ),
        0.98,
    ),
    (
        "MASRAF_BILGISI",
        re.compile(
            r"(?:(?:dosya|ekspertiz|swift)\s+(?:masrafı|ücreti)"
            r"(?:[^.!?\n]{0,60})?|masrafsız\s+(?:finansman|bankacılık|hesap|"
            r"para\s+transferi|döviz\s+havalesi|banka\s+ve\s+kredi\s+kartı)|"
            r"(?:masraf|ücret)\s+alınma(?:z|maktadır))",
            re.I,
        ),
        0.95,
    ),
    (
        "KAMPANYA_AVANTAJI",
        re.compile(
            r"(?:vade\s+farksız|ücretsiz\s+(?:hizmet|sigorta|ekspertiz)|"
            r"k[âa]r\s+payı\s+avantajı|ödeme\s+erteleme)",
            re.I,
        ),
        0.93,
    ),
    (
        "URUN_TURU",
        re.compile(
            r"(?:konut|taşıt|araç|ihtiyaç|alışveriş|eğitim|arsa|iş\s*yeri|tarım|"
            r"ticari|KOBİ|dijital|yeşil)\s+finansmanı|katılma\s+hesabı|"
            r"kira\s+sertifikası|(?:kredi|banka)\s+kartı|BES|Erken\s+BES",
            re.I,
        ),
        0.96,
    ),
...
CONDITION_PATTERNS = (
    re.compile(
        rf"(?:en\s+az|min(?:imum)?\.?\s*){MONEY}"
        rf"(?:[^.!?]{{0,80}}(?:harcama|alışveriş|işlem))?",
        re.I,
    ),
...
def annotate(text: str, bank_name: str) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for pattern in PROMO_CODE_PATTERNS:
        for match in pattern.finditer(text):
            ...
            if code.upper() not in PROMO_CODE_STOPWORDS:
                _add_range(
                    spans,
                    occupied,
                    text,
...
def prepare(input_path: str | Path, output_path: str | Path) -> dict[str, Any]:
    ...
    for index, row in enumerate(rows):
        ...
        record_id = str(row.get("id") or f"campaign-{index}")
        family_id = source_family_id(
            {"id": record_id, "source_url": row.get("source_url")}
        )
        prepared.append(
            {
                "id": record_id,
                ...
                "labeling_method": "terminology-regex-2026-08",
            }
        )
