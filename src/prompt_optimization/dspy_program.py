...
class CampaignAnswer(dspy.Signature):
    ...
    Kampanya metni ile sınıflandırma ve entity kanıtlarını birlikte kullan.
    Sadece verilen kanıtlara dayan; bilgi yoksa tam olarak
    'Bu bilgi sağlanan dokümanlarda bulunmamaktadır.' de. Birbiriyle çelişen
    kanıtlarda ham kampanya metnini esas al. Sayı, oran, tarih, kod ve koşulları
    aynen koru. Kısa, açık ve kullanıcı sorusuna doğrudan cevap ver.
    """
    ...
    classification_json: str = dspy.InputField(
        desc="Sınıflandırma modelinin yapılandırılmış çıktısı"
    )
