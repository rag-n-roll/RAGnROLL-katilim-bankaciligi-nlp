...
def offer(identifier: str, *, rate=None, amount=None, currency="TRY", product_type="financing", title="Finansman"):
    structured = {"product_type": product_type, "financing_type": "consumer", "profit_share_rate": rate,
                  "max_amount": amount, "fee_information": "masrafsız", "target_audience": "new_customer",
                  "duration": {"value": 3, "unit": "month", "approx_days": 90}}
    return {"id": identifier, "title": title, "structured": structured,
            "currency": currency}
...
def test_comparison_reuses_sorted_pair_key_once():
    ...
    results = compare_records(records, ComparisonQuery(product_type="financing", currency="TRY"))
...
def test_matching_score_uses_optional_structured_and_title_filters():
    results = compare_records(
        [offer("match", rate=0.35, amount={"amount": 100000, "currency": "TRY"}, title="İhtiyaç Finansmanı"),
         offer("different", rate=0.35, amount={"amount": 100000, "currency": "TRY"}, title="Taşıt Finansmanı")],
        ComparisonQuery(product_type="financing", currency="TRY", financing_type="consumer",
                        amount=100000, duration_days=90, eligibility="new_customer", title="İhtiyaç Finansmanı"),
    )
...
def test_comparison_returns_serializable_result_contract():
    result = compare_records([offer("one", rate=0.35)], ComparisonQuery(product_type="financing", currency="TRY"))
