...
from src.chatbot.prompt_config import (
    ...
    classifier = [
        {
            "id": "abc",
            "text": "Temiz kampanya",
            "annotations": {"product_category": "card"},
        }
    ]
    ner = [
        {
            "id": "abc",
            "text": "Temiz kampanya",
            "entities": [{"label": "BANKA", "text": "Örnek Bank"}],
        }
    ]
    ...
    ner = [
        {
            "id": "only-ner",
            "text": "B",
            "entities": [{"label": "BANKA", "text": "B Bank"}],
        }
    ]
    examples = build_examples(classifier, ner)
...
def test_prompt_config_fallback_and_few_shot_rendering(tmp_path):
    missing_config = load_prompt_config(tmp_path / "missing.json")
    assert missing_config["system_prompt"] == DEFAULT_SYSTEM_PROMPT
