...
export default function FloatingAiLink() {
  ...
  return (
    <Link draggable={false} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={stopDragging} onPointerCancel={stopDragging}
      onClick={(event) => { if (drag.current.moved) { event.preventDefault(); drag.current.moved = false; } }}
      style={position ? { left: position.x, top: position.y, right: "auto", bottom: "auto" } : undefined}
      className={styles.floatingAi} href="/chatbot" aria-label="Pusula AI sayfasını aç veya sürükleyerek taşı">
